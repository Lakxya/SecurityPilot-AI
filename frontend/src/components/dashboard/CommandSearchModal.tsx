...
export function CommandSearchModal({ isOpen, onClose }: CommandSearchModalProps) {
  ...
  return (
    <Dialog isOpen={isOpen} onClose={onClose} maxWidth="lg" className="p-0 overflow-hidden font-sans">
            ...
            filtered.map((cmd, idx) => {
              ...
              return (
                <button
                  key={cmd.title}
                  onClick={onClose}
                  className={`w-full p-2.5 rounded-xl flex items-center justify-between transition-all duration-150 text-left cursor-pointer ${
                    isSelected
                      ? 'bg-indigo-600/25 border border-indigo-500/40 text-indigo-200 shadow-md'
